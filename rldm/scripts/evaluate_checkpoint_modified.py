...
Example usage:
    ...
    Test a checkpoint for 100 episodes:
        python -m rldm.scripts.evaluate_checkpoint_modified -c mnt/rldm/scripts/checkpoints.json -e 100 -g
...
def main(checkpoint, algorithm, env_name, config, num_episodes, debug):
    ...
    eps_stats["statics/teammate_spread_out"] = []
    eps_stats["statics/teammate_pass_team_1"] = []
    eps_stats["statics/teammate_pass_team_2"] =[]
    eps_stats["statics/teammate_interception_team_1"] = []
    eps_stats["statics/teammate_interception_team_2"] = []
    eps_stats["statics/teammate_pass_interception_rate_team_1"] = []
    eps_stats["statics/teammate_pass_interception_rate_team_2"] = []
    eps_stats["statics/players_tired_factor_rate"] =[]
    ...
    for eidx in range(num_episodes):
        ...
        eps_stats["statics/teammate_spread_out"].append(0)
        eps_stats["statics/teammate_pass_team_1"].append(0)
        eps_stats["statics/teammate_pass_team_2"].append(0)
        eps_stats["statics/teammate_interception_team_1"].append(0)
        eps_stats["statics/teammate_interception_team_2"].append(0)
        eps_stats["statics/teammate_pass_interception_rate_team_1"].append(0)
        eps_stats["statics/teammate_pass_interception_rate_team_2"].append(0)
        eps_stats["statics/players_tired_factor_rate"].append(0)

        ball_owned = []
        player_1_tired_factor = 0
        player_2_tired_factor = 0
        team_1_pass = 0
        team_2_pass = 0
        team_1_interception = 0
        team_2_interception = 0
        spread_out = 0
        ...
        while not done:
            ...
            # if len(episode.get_agents())>1:
            if True:
                agent_ids = ["player_0","player_1"]
                info_1 = infos[agent_ids[0]]
                info_2 = infos[agent_ids[1]]
                ...
                spread_out += (val_1+val_2)**0.5

                
                player_1_tired_factor += info_1['left_team_tired_factor'][0]
                player_2_tired_factor += info_2['left_team_tired_factor'][1]
                ...
                if player != 0 and ((len(ball_owned)==0) or (len(ball_owned)>0 and player != ball_owned[-1])):
                    ball_owned.append(player)
                if len(ball_owned)>1:

                    if ball_owned[-2] <3 and ball_owned[-1] <3:
                        team_1_pass += 1
                    elif ball_owned[-2] >=3 and ball_owned[-1] >=3:
                        team_2_pass += 1
                    elif ball_owned[-2] >=3 and ball_owned[-1] <3:
                        team_1_interception += 1
                    elif ball_owned[-2] <3 and ball_owned[-1] >=3:
                        team_2_interception += 1
        ...
        if player_2_tired_factor != 0:
            val_tired_factor_rate = player_1_tired_factor/player_2_tired_factor
        else:
            val_tired_factor_rate = 0
        eps_stats["statics/teammate_spread_out"][-1] = spread_out
        eps_stats["statics/teammate_pass_team_1"][-1] = team_1_pass
        eps_stats["statics/teammate_pass_team_2"][-1] = team_2_pass
        eps_stats["statics/teammate_interception_team_1"][-1] = team_1_interception
        eps_stats["statics/teammate_interception_team_2"][-1] = team_2_interception
        eps_stats["statics/teammate_pass_interception_rate_team_1"][-1] = team_1_pass/team_1_interception if team_1_interception!=0 else 0.5
        eps_stats["statics/teammate_pass_interception_rate_team_2"][-1] = team_2_pass/team_2_interception if team_2_interception!=0 else 0.5
        eps_stats["statics/players_tired_factor_rate"][-1] = val_tired_factor_rate
        print(ball_owned)
...
def createBarplot(data1,data2):
    for y_title in list(data2.keys()):
        data = data1
        data[y_title] = data2[y_title]  
        
        df = pd.DataFrame(data)
        sns.barplot(x='Checkpoint', y= y_title, capsize=0.2, data=df).set(title= y_title + " for each Checkpoint")
        plt.xticks(rotation=40)
        plt.tight_layout()
        plt.savefig('/mnt/logs/plots/'+y_title+'.png')
        plt.clf()
...
if __name__ == '__main__':
    ...
    data2 = {'Win Rate':[], 
        ...
        "Teammate Spread Out":[],
        "Teammate Pass Number":[],
        "Teammate Interception Number":[],
        "Teammate Pass Interception Rate":[],
        "Players Tired Factor Rate":[],
        }
    for name, check in checkpoints.items():
        ...
        if not args.dry_run:
            ...
            if args.graph:
                ...
                data1['Checkpoint'].extend([name for _ in range(data_len)])
                # Y Axis
                data2['Win Rate'].extend(output['win_perc'])
                data2['Score Reward'].extend(output['score_reward'])
                data2['Undefeated Percentage'].extend(output['undefeated_perc'])

                data2["Teammate Spread Out"].extend(output['statics/teammate_spread_out'])
                data2["Teammate Pass Number"].extend(output['statics/teammate_pass_team_1'])
                data2["Teammate Interception Number"].extend(output['statics/teammate_interception_team_1'])
                data2["Teammate Pass Interception Rate"].extend(output['statics/teammate_pass_interception_rate_team_1'])
                data2["Players Tired Factor Rate"].extend(output['statics/players_tired_factor_rate'])
    createBarplot(data1,data2)
