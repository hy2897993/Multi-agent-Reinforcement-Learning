...
from ray.rllib.agents.impala.impala import ImpalaTrainer
from ray.rllib.agents.impala.vtrace_tf_policy import VTraceTFPolicy, setup_mixins as setup_tf_mixins, \
    build_vtrace_loss as tf_loss
from ray.rllib.agents.impala.vtrace_torch_policy import VTraceTorchPolicy, \
    setup_mixins as setup_torch_mixins, build_vtrace_loss as torch_loss

from ray.rllib.agents.ppo.ppo_tf_policy import KLCoeffMixin
from ray.rllib.agents.ppo.ppo_torch_policy import KLCoeffMixin as TorchKLCoeffMixin
...
CCIMPALATFPolicy = VTraceTFPolicy.with_updates(
    name="CCIMPALATFPolicy",
...
CCIMPALATorchPolicy = VTraceTorchPolicy.with_updates(
    name="CCIMPALATorchPolicy",
    postprocess_fn=centralized_critic_postprocessing,
    loss_fn=loss_with_central_critic,
...
def get_policy_class(config):
    if config["framework"] == "torch":
        return CCIMPALATorchPolicy


CCTrainer = ImpalaTrainer.with_updates(
    name="CCIMPALATrainer",
    default_policy=CCIMPALATFPolicy,
...
def main(n_cpus, n_gpus, env_name,
    ...
    config = {
            ...
            "simple_optimizer": 1,
            "vtrace": True,
            "lr_schedule": [[0, 0.00005], [5000000, 1e-12]],
