...
def main(n_cpus, n_gpus, env_name,
    ...
    policies = {
        ...
        # gen_policy(idx) for idx in range(n_policies)
    }
    ...
    obs_space_group = Tuple([obs_space['player_0'], obs_space['player_1']])
    act_space_group = Tuple([act_space['player_0'], act_space['player_1']])
 
    register_env(env_name, lambda _: ft.RllibGFootball(env_name=env_name)
    .with_agent_groups(
            grouping, obs_space=obs_space_group, act_space=act_space_group
        )
    )
